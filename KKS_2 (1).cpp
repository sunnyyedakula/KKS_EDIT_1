...
template <int dim, typename T> void update(grid<dim,vector<T> >& oldGrid, int steps)
{
	...
	grid<dim,vector<T> > temp(oldGrid);
	double dV=1.0;
	for (int d=0; d<dim; d++) {
	...
	for (int step=0; step<steps; step++) {
		...
		for (int n=0; n<nodes(oldGrid); n++) {
                    vector <T> grad = gradient(oldGrid, n, 0);
                    const double magGradPhi = grad[0] * grad[0] + grad[1] * grad[1];
                    const double sk = std::sqrt(magGradPhi);
                    const double invMagGradPhi = std::fabs(sk > 1e-12) ? 1.0/sk : 0.0; // catch divide-by-zero errors
                    vector<double> k = invMagGradPhi * grad; // normalize gradPhi
                   // const double k0 = (double) k[0];
                    //const double k1 = (double) k[1];
                    //temp(n) = 0.5 * (k0 * k0 * k0 * k0  + k1 * k1 * k1 * k1) + (k0 * k0 * k1 * k1);
                    temp(n) = 0.5 * (k[0] * k[0] * k[0] * k[0]  + k[1] * k[1] * k[1] * k[1]) + (k[0] * k[0] * k[1] * k[1]);
		}
...
*/
			...
			newGrid(n)[0] = phi_old + dt*temp(n)[0]*(eps_sq*lapPhi - omega*gprime(phi_old)
			...
			double myv = 0.0;
			if (newGrid(n)[0]>0.03 && newGrid(n)[0]<0.07) {
				gradPsq = 0.0;
				for (int d=0; d<dim; d++) {
...
double dCl_dc(const double p, const double Cs, const double Cl)
{
   ...
   /* template <typename T> MMSP::vector<T> gradPhi = MMSP::gradient(oldGrid, x, 0);

    double magGradPhi = std::sqrt(grad[0] * grad[0] + grad[1] * grad[1]);
...
double L(const double a, const double b) 

        return b11 * (a * a * a * a + b * b * b * b) + b00 * (a * a * b * b);*/
